import numpy as np
import random
from model.loss import model_loss
...
class STEDR(nn.Module):
    def __init__(self, config):
        ...
        self.criterion = partial(model_loss)
        
        self.attention_layers = nn.ModuleList([nn.Linear(n_seqs, att_dim) for _ in range(self.input_dim)])
    ...
    def get_features(self, x, x_lengths=None):
        ...
        weights = self.calc_sim(global_z_, cluster_z_)  # compute all cluster simiarities to global dist
        ...
        loss = mse_loss + kl_loss + target_dist_loss #+ SupCont_loss
    ...
    def predict(self, x, t, y, lengths=None):
        ...
        SupCont_loss = self.SupCont(global_z_, c_pred)
        
        all_loss = pred_loss + loss + c_loss+ SupCont_loss + self.alpha*(overlap)
